...
public class HqlGenerateUtil {
	...
	private static void installHqlJoinAlias(CriteriaQuery cq, Object searchObj, Map<String, TSDataRule> ruleMap, Map<String, String[]> parameterMap, String alias) {
		...
		for (int i = 0; i < origDescriptors.length; i++) {
			...
			try {
				...
				boolean isTransient = origDescriptors[i].getReadMethod().isAnnotationPresent(Transient.class);
				if (isTransient) {// 排除ORM忽略的字段
					continue;
				}
	...
	private static Map<String, TSDataRule> getRuleMap() {
		...
		List<TSDataRule> list = JeecgDataAutorUtils.loadDataSearchConditonSQL(); // (List<TSDataRule>)
																					// ContextHolderUtils
