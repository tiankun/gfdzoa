...
//			}
			...
			List<TSRole> list = systemService.findHql("select TSRole from TSRoleUser t  where t.TSUser.id = ? ", user.getId());
			for (int i = 0; i < list.size(); i++) {
				roles += list.get(i).getRoleName() + ",";
			}
...
//			}
			...
			//根据用户id去用户角色
			String roleByuseidhql = "select t1.TSRole from TSRoleUser t1 where t1.TSUser.id = ?)";
			List<TSRole> listRole = systemService.findHql(roleByuseidhql, user.getId());
			for (TSRole ru : listRole) {
                assembleFunctionsByRole(loginActionlist, ru);
			}
...
//            }
          ...
          //根据用户部门id去用户角色
			String roleByDidhql = "select t.tsRole from TSRoleOrg t where t.tsDepart.id = ?)";
			List<TSRole> listRoles = systemService.findHql(roleByDidhql, orgId);
			for (TSRole ru : listRoles) {
                assembleFunctionsByRole(loginActionlist, ru);
			}
...
//        }
        ...
        String hql = "from TSFunction t left join fetch t.TSFunctions s where t.id in(select r.TSFunction.id from TSRoleFunction r where r.TSRole.id=?)";
		List<TSFunction> list = systemService.findHql(hql, role.getId());
		for (int i = 0; i < list.size(); i++) {
			  if(list.get(i).getFunctionType().intValue()==Globals.Function_TYPE_FROM.intValue()){
					//如果为表单或者弹出 不显示在系统菜单里面
					continue;
			  }
			loginActionlist.put(list.get(i).getId(), list.get(i));
		}
    }
