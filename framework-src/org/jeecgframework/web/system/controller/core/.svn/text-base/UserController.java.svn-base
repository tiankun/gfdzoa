...
public class UserController extends BaseController {
	...
	protected void saveRoleUser(TSUser user, String roleidstr) {
		...
		for (int i = 0; i < roleids.length; i++) {
			...
			identityService.saveUser(identityService.newUser(user.getId()));
			identityService.createMembership(user.getId(), role.getRoleCode());// 绑定
...
//	}
	...
	@RequestMapping(params = "departManager")
	public String departManager(HttpServletRequest request) {
		return "system/user/departManager";
	}

	@RequestMapping(params = "managerLeader")
	public String managerLeader(HttpServletRequest request) {
		return "system/user/managerLeader";
	}

	@RequestMapping(params = "chooseUserDatagrid")
	public void chooseUserDatagrid(HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		CriteriaQuery cq = new CriteriaQuery(TSUser.class, dataGrid);
		this.systemService.getDataGridReturn(cq, true);
		TagUtil.datagrid(response, dataGrid);
	}
