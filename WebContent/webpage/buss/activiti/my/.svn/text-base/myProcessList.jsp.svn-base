...
<table id="myprocessList" style="width: 700px; height: 300px">
	<thead>
		<tr>
			...
			<th field="deploymentId" hidden="hidden">编号</th>
		...
		function getData(data){
			...
			for(var i=0; i<data.rows.length; i++){
				rows.push({
					...
					opt: "[<a href=\"#\" onclick=\"add('发起新流程','activitiController.do?startPageSelect&deploymentId="+data.rows[i].deploymentId+"','myprocessList')\">发起新流程</a>]"
				});
			}
