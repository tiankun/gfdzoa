		...
		function getData(data){
			...
			for(var i=0; i<data.rows.length; i++){
				rows.push({
					...
					opt: "[<a href=\"#\" onclick=\"add('办理','"+data.rows[i].action+"&processInstanceId="+data.rows[i].processInstanceId+"&taskId="+data.rows[i].taskId+"&taskKey="+data.rows[i].TaskDefinitionKey+"','claimedTask','100%','100%')\">办理</a>]"
				});
			}
