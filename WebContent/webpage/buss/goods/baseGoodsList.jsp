...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="baseGoodsList" checkbox="true" fitColumns="true" title="商品名称" pageSize="20"
...
<%--    <t:dgToolBar title="模板下载" icon="icon-putout" funname="ExportXlsByT"></t:dgToolBar> --%>
 ...
 <script src = "webpage/com/buss/goods/baseGoodsList.js"></script>		
 <script type="text/javascript">
 $(document).ready(function(){
 		//给时间控件加上样式
 			$("#baseGoodsListtb").find("input[name='createDate']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#baseGoodsListtb").find("input[name='updateDate']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 });
 
//导入
function ImportXls() {
	openuploadwin('Excel导入', 'goodsController.do?upload', "baseGoodsList");
}

//导出
function ExportXls() {
	JeecgExcelExport("goodsController.do?exportXls","baseGoodsList");
}

//模板下载
function ExportXlsByT() {
	JeecgExcelExport("goodsController.do?exportXlsByT","baseGoodsList");
}
 </script>