...
function cost(){
	 var total=0;
	  $("#goodsList tr").each(function(){
		  var num = 0;
		  if($(this).children().eq(6).children().eq(0).val()!=""){
			  num = $(this).children().eq(6).children().eq(0).val();
		  }
		  $(this).children().eq(7).children().eq(0).val(($(this).children().eq(5).children().eq(0).val()*1)*(num*1));
		  total+=$(this).children().eq(7).children().eq(0).val()*1;
	  });
...
function addCost(){
 ...
 <body>
  <form name="formobj" id="formobj" action="orderController.do?updateCgOrder">
  ...
  <div id="agent" class="easyui-panel" title="客户订单" style="width:auto;height:auto;margin-bottom:1px;" data-options="doSize:false">
	<table cellpadding="0" cellspacing="1" class="formtable">
		<tr>
			...
			<td class="value">
			<input type="hidden" id="id" name ="id" value="${orderPage.id }">
			<input type="text" readonly="readonly" class="inputxt" id="orderNumber" name="orderNumber" value="${orderPage.orderNumber }" datatype="*">
			...
			<td class="value">
				<input class="inputxt" id="orderDate" name="orderDate"  class="Wdate" onClick="WdatePicker()" value='<fmt:formatDate value="${orderPage.orderDate }" pattern="yyyy-MM-dd"/>'  style="width: 150px">
				<span class="Validform_checktip"></span>
			</td>
		</tr>
	</table>
	...
	<div id="goodsDiv" style="width: auto;height:290px;padding:0px 3px 0px 3px;overflow: auto;">
	  <table id="blist" style="width:100%;table-layout:fixed;margin-top:-1px" cellpadding="0" cellspacing="1" class="formList">
		...
		<c:forEach items="${list}" var="pd" varStatus="pdIndex">
		  ...
		  <td width="150px"><input type="hidden" name="orderDetail[${pdIndex.index+1}].goods.id" value="${pd.goods.id}">${pd.goods.goodsName}</td>
		  <td width="150px">${pd.goods.goodsModel}</td>
...
function saveData(){
	$.post('orderController.do?updateCgOrder',$("#formobj").serialize(),function(data){
