...
@Service("zhbPlancpService")
@Transactional
public class ZhbPlancpServiceImpl extends CommonServiceImpl implements ZhbPlancpServiceI {
	
 	public <T> void delete(T entity) {
 		super.delete(entity);
 		//执行删除操作配置的sql增强
		this.doDelSql((ZhbPlancpEntity)entity);
 	}
	...
	public void addMain(ZhbPlancpEntity zhbPlancp,
			...
			for(ZhbPlanlistEntity zhbPlanlist:zhbPlanlistList){
				//外键设置
				zhbPlanlist.setPlancpId(zhbPlancp.getId());
				/*//外键设置
				zhbPlanlist.setPlanaditId(zhbPlancp.getId());*/
				this.save(zhbPlanlist);
			}
			//执行新增操作配置的sql增强
 			this.doAddSql(zhbPlancp);
	}
	...
	public void updateMain(ZhbPlancpEntity zhbPlancp,
	        List<ZhbPlanlistEntity> zhbPlanlistList) {
		//保存主表信息
		this.saveOrUpdate(zhbPlancp);
		//===================================================================================
		//获取参数
		Object id0 = zhbPlancp.getId();
		//Object id0 = zhbPlancp.getId();
		//===================================================================================
		//1.查询出数据库的明细数据-采购申请材料详细
	    String hql0 = "from ZhbPlanlistEntity where 1 = 1 AND pLANCP_ID = ?   ";
	    List<ZhbPlanlistEntity> zhbPlanlistOldList = this.findHql(hql0,id0);
		//2.筛选更新明细数据-采购申请材料详细
		for(ZhbPlanlistEntity oldE:zhbPlanlistOldList){
			boolean isUpdate = false;
	    		...
	    		if(!isUpdate){
		    		//如果数据库存在的明细，前台没有传递过来则是删除-采购申请材料详细
		    		super.delete(oldE);
	    		}
		...
		//执行更新操作配置的sql增强
 		this.doUpdateSql(zhbPlancp);
	}

	
	public void delMain(ZhbPlancpEntity zhbPlancp) {
		//删除主表信息
		this.delete(zhbPlancp);
		//===================================================================================
		//获取参数
		Object id0 = zhbPlancp.getId();
		//Object id0 = zhbPlancp.getId();
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,ZhbPlancpEntity t){
 		...
 		sql  = sql.replace("#{apply_person}",String.valueOf(t.getApplyPerson()));
 		sql  = sql.replace("#{plan_time}",String.valueOf(t.getPlanTime()));
 		sql  = sql.replace("#{plan_explain}",String.valueOf(t.getPlanExplain()));
